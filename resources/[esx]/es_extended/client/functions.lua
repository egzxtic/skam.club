...
ESX.Game = {}
ESX.Game.Utils = {}
ESX.Game.Utils.MessagesFloating = {}
ESX.Scaleform = {}
ESX.Scaleform.Utils = {}
ESX.Streaming = {}
ESX.Binds = {}
...
function ESX.ShowNotification(description, variant, title)
    if GetResourceState("skam-ui") ~= "missing" then
        exports['skam-ui']:showNotification({ description = description, variant = variant, title = title })
    else
        print("[ERROR] Missing skam-ui")
    end
end
...
function ESX.UI.ChoosePlayerMenu()

    local elements = {}

    local playersInArea = ESX.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 3.0)
    if #playersInArea > 0 then
        for i, player in ipairs(playersInArea) do
            elements[i] = {label = GetPlayerServerId(player), value = GetPlayerServerId(player)}
        end
    else
        return ESX.ShowNotification('Brak graczy w pobliżu', 'error')
    end

    local currentPlayer, choosedPlayer = GetPlayerFromServerId(elements[1].value), nil

    CreateThread(function()
        while currentPlayer do
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'choose_player_menu', {
		title    = 'Wybierz gracza',
		align    = 'right',
		elements = elements
	}, function(data, menu)
        menu.close()
        playersNearby = ESX.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 3.0)
        playersNearby = ESX.Table.Set(playersNearby)
        if playersNearby[GetPlayerFromServerId(data.current.value)] then
            currentPlayer, choosedPlayer = nil, data.current.value
        else
            ESX.ShowNotification('Gracz się oddalił', 'error')
			currentPlayer, choosedPlayer = nil, false
        end
	end, function(data, menu)
		menu.close()
        currentPlayer, choosedPlayer = nil, false
...
for _, weapon in ipairs(Config.Weapons) do
    if weapon.item then
        ...
        weaponLabels[weapon.item] = weapon.label or weapon.item
    end
end
...
RegisterCommand('slot1', function()	
    if not IsControlPressed(0, Keys['LEFTALT']) then
        ESX.UseBind(1)
    end
end)
...
function ESX.SaveBinds()
    if GetResourceState('skam-ui') == 'started' then
        exports['skam-ui']:updateBinds(ESX.Binds);
    end
	SendNUIMessage({
		action = "saveBinds",
		binds = ESX.Binds
...
RegisterNUICallback('updateBinds', function(data)
	if not data then
		return
	end
...
function ESX.ShowInventory()
    ...
    for i=1, #(ESX.PlayerData.accounts) do
        if ESX.PlayerData.accounts[i].money > 0 then
            ...
            table.insert(elements, {
                label = ('%s: <span style="color:#999999;">%s</span>'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
    ...
    for k, v in ipairs(ESX.PlayerData.inventory) do
        if v.count > 0 then
            ...
            local label = ('%s x%s'):format(v.label, v.count)

            local bind = ESX.GetBindItemControl(v.name)
            if bind then
                label = (label .. ('<span style="float: right; margin-right:.5vh; color: %s;">%s</span>'):format("#999999", bind))
            end
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
        ...
        if useAllItems[data.current.value] and data.current.usable and data.current.count > 1 then
            table.insert(elements, {
                label = 'Użyj wszystko',
                action = 'use_all',
                type = data.current.type,
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
            ...
            elseif data1.current.action == 'use_all' then
                TriggerServerEvent('esx:useItemAll', item)
...
AddEventHandler('esx:showNotification', function(description, variant, title)
    ESX.ShowNotification(description, variant, title)
end)
