function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, dualjob, loadout, name, coords)
	...
	self.discord = ESX.getDiscordData(playerId);
	...
	function self.setDualJob(dualjob, grade)
		grade = tonumber(grade)
		local lastDualJob = json.decode(json.encode(self.dualjob))

		if ESX.DoesJobExist(dualjob, grade) then
			local jobObject, gradeObject = ESX.Jobs[dualjob], ESX.Jobs[dualjob].grades[grade]

			self.dualjob.id    = jobObject.id
			self.dualjob.name  = jobObject.name
			self.dualjob.label = jobObject.label

			self.dualjob.grade        = tonumber(grade)
			self.dualjob.grade_name   = gradeObject.name
			self.dualjob.grade_salary = gradeObject.salary
			self.dualjob.grade_permissions = gradeObject.permissions

			TriggerEvent('esx:setDualJob', self.source, self.dualjob, lastDualJob)
			self.triggerEvent('esx:setDualJob', self.dualjob)
		else
			print(('[es_extended] [^3WARNING^7] Ignoring invalid .setDualJob() usage for "%s"'):format(self.identifier))
		end
	end
