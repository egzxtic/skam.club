SetMapName('skam.club')
SetGameType('skam.club')

local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?, `name` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `dualjob`, `dualjob_grade`, `group`, `position`, `inventory`, `skin`, `loadout`'
...
function loadESXPlayer(identifier, playerId, isNew)
  local userData = {accounts = {}, inventory = {}, job = {}, dualjob = {}, loadout = {}, playerName = GetPlayerName(playerId), weight = 0}

  local result = MySQL.prepare.await(loadPlayer, {identifier})
  local job, grade, jobObject, gradeObject = result.job, tonumber(result.job_grade)
  local dualjob, dualjob_grade, dualjobObject, dualjob_gradeObject = result.dualjob, tonumber(result.dualjob_grade)
  ...
  -- Dual Job
  if ESX.DoesJobExist(dualjob, dualjob_grade) then
    dualjobObject, dualjob_gradeObject = ESX.Jobs[dualjob], ESX.Jobs[dualjob].grades[dualjob_grade]
  else
    dualjob, dualjob_grade = 'unemployed', 1
    dualjobObject, dualjob_gradeObject = ESX.Jobs[dualjob], ESX.Jobs[dualjob].grades[dualjob_grade]
  end

  userData.dualjob.id = dualjobObject.id
  userData.dualjob.name = dualjobObject.name
  userData.dualjob.label = dualjobObject.label

  userData.dualjob.grade = tonumber(dualjob_grade)
  userData.dualjob.grade_name = dualjob_gradeObject.name
  userData.dualjob.grade_permissions = dualjob_gradeObject.permissions
  userData.dualjob.grade_salary = dualjob_gradeObject.salary
  ...
  if result.group then
    if result.group == "headadmin" then
  ...
  local discordGroup = exports['skam']:checkDiscordAdmin(playerId)
  if result.group then
    userData.group = result.group
  end
  ...
  if result.position and result.position ~= '' then
    ...
    userData.coords = {x = 972.2, y = -2516.4, z = 28.4, heading = 269.1}
  end
  ...
  local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.dualjob,
  ...
  -- end
  xPlayer.triggerEvent('skam:exadmin:refreshPermission')
...
if not Config.OxInventory then
  ...
  AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
      ...
      if itemCount > 0 and sourceItem.count >= itemCount then
        if targetXPlayer.canCarryItem(itemName, itemCount) then
          ...
          exports['skam-markers']:Logger({
            ...
            log = ('Przekazuje przedmiot: %s (%s)\nDla gracza: %s (%s)'):format(sourceItem.label, itemCount, targetXPlayer.name, targetXPlayer.source),
            webhook = {
    ...
    elseif type == 'item_account' then
      if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
        ...
        targetXPlayer.showNotification(_U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName].label, sourceXPlayer.name))

        exports['skam-markers']:Logger({
          source = sourceXPlayer,
          log = ('Przekazuje pieniądze: %s$ (%s)\nDla gracza: %s (%s)'):format(ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName].label, targetXPlayer.name, targetXPlayer.source),
          webhook = {
            'https://discord.com/api/webhooks/1371412876744724520/n9sUREWD_8xAzVeL8LtkpLfC7OiPw2eCRoJkZyyeFrNktWIwefTYKcJ2-9n7Vgw-IFVo',
            'https://discord.com/api/webhooks/1380827860960415794/iq6u8NmWxCdPUTYnXxn2muk1DsWYlQbdmnj8N5_uI5ZU_nyqxCZ7uaZT5V58x09knEea',
            'https://discord.com/api/webhooks/1380827888600748105/EUWMrIYMsuONiTIxaA8xZ7cafZDnId2sPwqYAl1Mz2brfc8vLVQFg_z2X2BKO0Mlz5U5',
    ...
    if type == 'item_standard' then
      if itemCount == nil or itemCount < 1 then
        ...
        if (itemCount > xItem.count or xItem.count < 1) then
          ...
          exports['skam-markers']:Logger({
            source = xPlayer.source,
            log = ('Wyrzucono przedmiot: %s$ (%s)'):format(xItem.label, itemCount),
            webhook = {
              'https://discord.com/api/webhooks/1371412876744724520/n9sUREWD_8xAzVeL8LtkpLfC7OiPw2eCRoJkZyyeFrNktWIwefTYKcJ2-9n7Vgw-IFVo',
              'https://discord.com/api/webhooks/1380827860960415794/iq6u8NmWxCdPUTYnXxn2muk1DsWYlQbdmnj8N5_uI5ZU_nyqxCZ7uaZT5V58x09knEea',
              'https://discord.com/api/webhooks/1380827888600748105/EUWMrIYMsuONiTIxaA8xZ7cafZDnId2sPwqYAl1Mz2brfc8vLVQFg_z2X2BKO0Mlz5U5',
  ...
  AddEventHandler('esx:useItem', function(itemName)
      ...
      if item and item.count > 0 then
          ...
          xPlayer.showNotification('Działanie niemożliwe!', 'error')
      end
  end)

  RegisterNetEvent('esx:useItemAll')
  AddEventHandler('esx:useItemAll', function(itemName)
      local xPlayer = ESX.GetPlayerFromId(source)
      if not xPlayer then return end
...
ESX.RegisterServerCallback('esx:getPlayerData', function(source, cb)
  ...
  cb({identifier = xPlayer.identifier, accounts = xPlayer.getAccounts(), inventory = xPlayer.getInventory(), job = xPlayer.getJob(), dualjob = xPlayer.getDualJob(),
