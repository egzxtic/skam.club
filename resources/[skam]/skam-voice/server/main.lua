...
local allowedVoiceModes = {
    [1] = 1.0,
    [2] = 6.0,
    [3] = 12.0
}

RegisterNetEvent('voice:requestProximity', function(requestedMode)
    local src = source
    local distance = allowedVoiceModes[requestedMode]
    if distance then
        TriggerClientEvent('voice:updateProximity', src, requestedMode, distance)
    else
		--exports['okokok']:fg_BanPlayer(_source, 'WOMP WOMP, WYPIERDALAJ', true)
        print(('[ANTICHEAT] Gracz %s próbował ustawić zły tryb głosu: %s'):format(src, tostring(requestedMode)))
        TriggerClientEvent('voice:updateProximity', src, 2, allowedVoiceModes[2]) -- default to normal
    end
end)